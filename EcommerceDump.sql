...
--
-- Temporary view structure for view `lowestexp_product`
--

DROP TABLE IF EXISTS `lowestexp_product`;
/*!50001 DROP VIEW IF EXISTS `lowestexp_product`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `lowestexp_product` AS SELECT 
 1 AS `CAT_ID`,
 1 AS `CAT_NAME`,
 1 AS `product_name`,
 1 AS `min_price`*/;
...
CREATE TABLE `order1` (
  ...
  CONSTRAINT `order1_ibfk_2` FOREIGN KEY (`PRICING_ID`) REFERENCES `supplier_pricing` (`PRICING_ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `rating` (
  ...
  PRIMARY KEY (`RAT_ID`),
  KEY `ORD_ID` (`ORD_ID`),
  CONSTRAINT `rating_ibfk_1` FOREIGN KEY (`ORD_ID`) REFERENCES `order1` (`ORD_ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
/*!50001 VIEW `lowest_expensive_product` AS select `product`.`PRO_ID` AS `PRO_ID`,`product`.`PRO_NAME` AS `PRO_NAME`,`order1`.`ORD_DATE` AS `ORD_DATE` from (`product` join `order1`) where (`order1`.`ORD_DATE` > '2021-10-05') */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;

--
-- Final view structure for view `lowestexp_product`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `lowestexp_product` AS select `category`.`CAT_ID` AS `CAT_ID`,`category`.`CAT_NAME` AS `CAT_NAME`,min(`product`.`PRO_NAME`) AS `product_name`,min(`supplier_pricing`.`SUPP_PRICE`) AS `min_price` from ((`category` join `product` on((`category`.`CAT_ID` = `product`.`CAT_ID`))) join `supplier_pricing` on((`product`.`PRO_ID` = `supplier_pricing`.`PRO_ID`))) group by `category`.`CAT_ID` order by `category`.`CAT_ID` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2024-01-21 19:23:15
